...
impl PasswordPolicy {
    fn new(line: String) -> PasswordPolicy {
        ...
        return PasswordPolicy { min: minMax[0], max: minMax[1], character: parts[1].replace(":", "").chars().next().unwrap(), password: parts[2].to_string() };
    }
}
...
fn main() {
    ...
    for pp in passwordPolicies.into_iter() {
        let valid_chars = pp.password.chars().filter(|c| *c == pp.character).count();
        if pp.min <= valid_chars && valid_chars <= pp.max {
            valid += 1;
        }
    }
